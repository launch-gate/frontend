...
const prizeInfoSchema = object({
  description: string().required("Описание приза обязательно"),
...
export const competitionItemSchema = object({
  ...
  registrationDateRange: array().required(
    "Диапазон дат регистрации обязателен",
  ),
  resultDateRange: array().required(
    "Диапазон дат подведения итогов обязателен",
  ),
  participantAgeRange: array().required("Возраст участников обязателен"),
  ...
  isCountry: boolean().required("Всероссийский или нет - обязательное поле"),
  managers: array().of(managerSchema).required("Менеджеры обязательны"),
  eventContacts: array()
    .of(eventContactSchema)
    .required("Контакты обязательны"),
});
